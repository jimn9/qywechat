package qyExternalTagGroupModel
...
type QyExternalTagGroup struct {
	models.BaseModel

	QyId       int       `gorm:"column:qy_id;type:int"`
	GroupId    string    `gorm:"column:group_id;type:varchar(60)"`
	GroupName  string    `gorm:"column:group_name;type:varchar(120)"`
...
func (QyExternalTagGroup) TableName() string {
	//实现TableName接口，以达到结构体和表对应，如果不实现该接口，并未设置全局表名禁用复数，gorm会自动扩展表名为articles（结构体+s）
	return "scrm_qy_external_tag_group"
}
